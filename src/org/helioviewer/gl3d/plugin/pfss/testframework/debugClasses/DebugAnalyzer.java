...
/**
 * This analyzes uncompressed with the lossy compressed data. This is useful for
 * debugging and developing the compresion.
 * 
...
public class DebugAnalyzer {
	...
	public static void analyze(ProcessedReader unc,
			CompressedDataReader comp, String csvFile) {
		...
		for (int i = 0; i < compLines.size(); i++) {
			...
			for (int j = 0; j < compLine.points.size(); j++) {
				...
				// out.write(
				// p1.getRawR()+";"+p1.getRawPhi()+";"+p1.getRawTheta()+";"+
				// p2.getRawR()+";"+p2.getRawPhi()+";"+p2.getRawTheta()+";"+errR+";"+errP+";"+errT);
				varR += errR * errR;
				varP += errP * errP;
				varT += errT * errT;
			}
		...
		System.out.println("Results:\tvariance\tmax");
		System.out.println("Radius:\t "+ varR+"\t"+maxErrR);
		System.out.println("Phi:\t "+ varP+"\t"+maxErrP);
		System.out.println("Radius:\t "+ varT+"\t"+maxErrT);
	}
}
