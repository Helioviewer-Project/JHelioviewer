...
public class LayerRayTrace {
	...
	public ImageRegion getCurrentRegion(MainPanel mainPanel, MetaData metaData,
			Dimension size) throws MetaDataException {
		/*
		 * if (!(mainPanel instanceof OverViewPanel)){ contentPanel.removeAll();
		 * contentPanel.setLayout(null); }
		 */
		rayTrace = new RayTrace(metaData.getRotation().toMatrix());

		double partOfWidth = mainPanel.getWidth() / (double) (MAX_X_POINTS - 1);
		double partOfHeight = mainPanel.getHeight()
				/ (double) (MAX_Y_POINTS - 1);
		...
		Rectangle2D rectangle = new Rectangle2D.Double(minX, minY, maxX - minX,
				maxY - minY);
