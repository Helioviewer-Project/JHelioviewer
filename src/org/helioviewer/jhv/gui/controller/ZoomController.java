...
public class ZoomController {
	...
	public void zoom(View topmostView, double zoomFactor) {
		...
		if (regionView != null && metaDataView != null
				&& metaDataView.getMetaData() != null) {

			// if zooming out, make sure that we do not get off too far
			if (zoomFactor < 1) {

				LayeredView layeredView = regionView
			...
			Vector2dDouble newSizeVector = Vector2dDouble.scale(
					oldRegion.getSize(), 1.0 / zoomFactor);

			Vector2dDouble newCorner = null;
			if (panel != null && viewportView != null) {
				Vector2dInt mousePosition = panel.getInputController()
						.getMousePosition();
				if (mousePosition != null) {
					Viewport v = viewportView.getViewport();
					ViewportImageSize vis = ViewHelper
							.calculateViewportImageSize(v, oldRegion);
					Vector2dInt visOffset = v.getSize()
							.subtract(vis.getSizeVector()).scale(0.5);
					Vector2dInt fixPointViewport = mousePosition
							.subtract(visOffset);
					if (fixPointViewport.getX() >= 0
							&& fixPointViewport.getY() >= 0) {
						Vector2dDouble fixPointOffset = ViewHelper
								.convertScreenToImageDisplacement(
										fixPointViewport
												.subtract(new Vector2dInt(0,
														vis.getHeight())),
										oldRegion, vis);
						Vector2dDouble fixPoint = fixPointOffset.add(oldRegion
								.getLowerLeftCorner());
						Vector2dDouble relativeFixPointOffset = fixPointOffset
								.invertedScale(oldRegion.getSize());
						Vector2dDouble newFixPointOffset = newSizeVector
								.scale(relativeFixPointOffset);
			...
			if (newCorner == null) {
				newCorner = Vector2dDouble.add(
						oldRegion.getLowerLeftCorner(),
						Vector2dDouble.scale(Vector2dDouble.subtract(
								oldRegion.getSize(), newSizeVector), 0.5));
			}
			Region zoomedRegion = ViewHelper.cropRegionToImage(
					StaticRegion.createAdaptedRegion(newCorner, newSizeVector),
					metaDataView.getMetaData());
