 ...
 * 
 */
public class NewMoviePanel extends JPanel implements ActionListener, ChangeListener, MouseListener, MouseWheelListener, ViewListener {
    ...
    private static LinkedList<NewMoviePanel> panelList = new LinkedList<NewMoviePanel>();
     ...
     */
    public NewMoviePanel(JHVJPXView movieView) {
        this();
    ...
    public NewMoviePanel() {
        ...
        previousFrameButton = ButtonCreator.createButton(ICON_BACKWARD, "Step to Previous Frame", this);
        buttonPanel.add(previousFrameButton);

        playPauseButton = ButtonCreator.createButton(ICON_PLAY, "Play movie", this);
        buttonPanel.add(playPauseButton);

        nextFrameButton = ButtonCreator.createButton(ICON_FORWARD, "Step to Next Frame", this);
        buttonPanel.add(nextFrameButton);
        secondLine.add(buttonPanel, BorderLayout.WEST);

        buttonPanel.add(new JSeparator(SwingConstants.VERTICAL));

        advancedButton = ButtonCreator.createTextButton(ICON_OPEN, "More Options", "More Options to Control Playback", this);
        ...
        this.setAdvanced(NewMoviePanel.isAdvanced);
    }
    ...
    public void setAdvanced(boolean advanced) {
        NewMoviePanel.isAdvanced = advanced;
     ...
     */
    public static NewMoviePanel getMoviePanel(JHVJPXView view) {
        if (view.getMaximumFrameNumber() <= 0) {
            return null;
        }

        synchronized (panelList) {
            for (NewMoviePanel moviePanel : panelList) {
    ...
    private static abstract class StaticMovieAction extends AbstractAction implements ActionListener, LayersListener {
        private static final long serialVersionUID = 1L;
        protected NewMoviePanel activePanel;
        ...
        private void searchCorrespondingMoviePanel(View view) {
            if (view != null) {
                ...
                if (JHVJPXView != null) {
                    setEnabled(true);
                    synchronized (panelList) {
                        for (NewMoviePanel panel : panelList) {
    ...
    private static class LinkedMovieManager {
        ...
        private LinkedList<NewMoviePanel> linkedMovies = new LinkedList<NewMoviePanel>();
         ...
         *            Panel to add
         */
        public void linkMoviePanel(NewMoviePanel newPanel) {
            newPanel.view.linkMovie();

            if (!linkedMovies.isEmpty()) {
                // Copy Settings
                NewMoviePanel copyFrom = linkedMovies.element();
         ...
         *            Panel to remove
         */
        public void unlinkMoviePanel(NewMoviePanel panel) {
         ...
         */
        public void updateSpeedSpinnerLinkedMovies(NewMoviePanel copyFrom) {
            for (NewMoviePanel panel : linkedMovies) {
         ...
         *            Panel dominating the other ones right now
         */
        public void updateSpeedUnitComboBoxLinkedMovies(NewMoviePanel copyFrom)
        {
         ...
         *            Panel dominating the other ones right now
         */
        public void updateAnimationModeComboBoxLinkedMovies(NewMoviePanel copyFrom)
        {
